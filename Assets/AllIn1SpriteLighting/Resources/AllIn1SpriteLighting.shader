Shader "AllIn1SpriteLighting/AllIn1SpriteLighting"
{
	Properties
	{
		...
		_Cutoff("Alpha cutoff", Range(0,1)) = 0.9 //3
		...
		_FadeTex("Fade Texture", 2D) = "white" {} //20
		...
		_FadeBurnGlow("Fade Burn Glow",  Range(1,50)) = 2//26

		_HsvShift("Hue Shift", Range(0, 360)) = 180 //27
		_HsvSaturation("Hue Shift Saturation", Range(0, 2)) = 1
		_HsvBright("Hue Shift Bright", Range(0, 2)) = 1 //29

		_HitEffectColor("Hit Effect Color", Color) = (1,1,1,1) //30
		_HitEffectGlow("Hit Effect Glow Intensity", Range(1,100)) = 5
		[Space]
		[Header((Tip) Animate the following property)]
		_HitEffectBlend("Hit Effect Blend", Range(0,1)) = 1 //32

		_ZWrite ("Depth Write", float) = 1 // 33
        _ZTestMode ("Z Test Mode", float) = 4 // 34

		_GrassSpeed("Speed", Range(0,50)) = 2 //35
		_GrassWind("Bend amount", Range(0,50)) = 20 //36
		_GrassRadialBend("Radial Bend", Range(0.0, 5.0)) = 0.1 //37

		_ExtraLight("Extra Light", Range(0.0, 1.0)) = 0.0 //38
	}

	SubShader
	{
		Tags{"Queue"="Geometry" "RenderType"="TransparentCutout" "PreviewType"="Plane" "CanUseSpriteAtlas"="True" "IgnoreProjector" = "True"}
		...
		CGPROGRAM
		#pragma target 3.0
		#pragma surface surf Custom addshadow fullforwardshadows
		...
		sampler2D _MainTex;
		half4 _Color;
		half _LightBoost, _Cutoff, _ExtraLight;
		...
		#if OUTLINE_ON
		half4 _OutlineColor;
		half _OutlineWidth, _OutlineAlpha, _OutlineGlow, _OutlineLit;
		#endif
		...
		half4 LightingCustom(SurfaceOutputCustom s, half3 lightDir, half3 viewDir, half atten) {
			...
			atten = saturate(_ExtraLight + atten);
